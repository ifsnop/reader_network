...
#endif
#ifdef SOLARIS
    log_printf(LOG_ERROR, "reader_rrd_SOL v%s\nsmrbarajasfix\tdestfiletimestamp\toutputcompress\n"
    "renicev2\tscrm(rbtrees/queues/crc32)\nfullstatsv2\ttodrolloverfix\t\tdecoder\n", VERSION);
#endif    
    startup();
...
char *tmp;
    ...
    tmp = mem_alloc(512);
    sprintf(tmp, "./rrd_create.sh %03d_%03d_%03d %ld", sac, sic, cat, midnight_t);
    system(tmp);
    mem_free(tmp);
    return;
}
...
//    log_printf(LOG_NORMAL, "%s %s\t%d\t%3.0f\t%3.3f\t%3.3f\t%3.3f\t%3.3f\t%3.3f\t%3.3f\n",
//        sac_s, sic_l, cat, cuenta, media, stdev, moda, max, min, p99);
//    log_printf(LOG_NORMAL, "%d %d\t%d\t%3.0f\t%3.3f\t%3.3f\t%3.3f\t%3.3f\t%3.3f\t%3.3f\n",
//        sac, sic, cat, cuenta, media, stdev, moda, max, min, p99);
...
//    } else { // calculo e insercion!
	tmp = mem_alloc(512);
    	sprintf(tmp, "./rrd_update.sh %03d_%03d_%03d %ld %3.3f %3.3f %3.3f %3.3f %3.3f", sac, sic, cat, 
	    timestamp, max, min, media, stdev, p99);
	system(tmp);
	mem_free(tmp);
//    	exit(EXIT_SUCCESS);
//    }
