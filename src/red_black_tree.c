...
/*  newTree->DestroyKey= DestFunc;
  ...
  newTree->AddQueue = AddQueue;
  newTree->DeleteQueue = DeleteQueue;
  newTree->count = 0;
...
void TreeInsertHelp(rb_red_blk_tree* tree, rb_red_blk_node* z) {
  ...
  while( x != nil) {
    y=x;
    if (1 == tree->Compare(x->crc32,z->crc32)) { /* x.key > z.key */
      x=x->left;
    } else { /* x,key <= z.key */
      x=x->right;
    }
  ...
  z->parent=y;
  if ( (y == tree->root) ||
       (1 == tree->Compare(y->crc32,z->crc32))) { /* y.key > z.key */
    y->left=z;
  } else {
    y->right=z;
  }
...
void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != tree->nil) {
    ...
    printf("  r->crc32=");
    if( x->right == nil) printf("NULL"); else printf("%08X", x->right->crc32);
    printf("  p->crc32=");
    if( x->parent == root) printf("NULL"); else printf("%08X", x->parent->crc32);
    printf("  red=%i\n",x->red);
...
rb_red_blk_node* RBExactQuery(rb_red_blk_tree* tree, unsigned int q) {
  rb_red_blk_node* x=tree->root->left;
  rb_red_blk_node* nil=tree->nil;
  int compVal;
  if (x == nil) return(0);
  compVal=tree->Compare(x->crc32, q);
...
stk_stack* RBEnumerate(rb_red_blk_tree* tree, unsigned int low, unsigned int high) {
  ...
  while(nil != x) {
    if ( 1 == (tree->Compare(x->crc32,high)) ) { /* x->key > high */
      x=x->left;
    } else {
      lastBest=x;
      x=x->right;
  ...
  while ( (lastBest != nil) && (1 != tree->Compare(low,lastBest->crc32))) {
