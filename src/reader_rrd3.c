...
long timestamp = 0;
	...
	log_printf(LOG_ERROR, "usage: %s <gps_file> [unix timestamp tod at 00:00:00]\n\n", argv[0]);
	exit(EXIT_FAILURE);
    }
    parse_config(/*argv[1]*/);
...
div_t d;
	...
	if (diff<-86000) diff+=86400; // rollover tod correction
...
char *tmp;
    ...
    tmp = mem_alloc(512);
    sprintf(tmp, "rrd_create3.sh %03d_%03d_%03d %ld 2> /dev/null", sac, sic, cat, midnight_t);
    system(tmp);
    mem_free(tmp);
    return;
}
...
//    } else { // calculo e insercion!
	tmp = mem_alloc(512);
    	sprintf(tmp, "rrd_update3.sh %03d_%03d_%03d %ld %3.3f %3.3f %3.3f %3.3f %3.3f %3.3f 2> /dev/null", sac, sic, cat, 
